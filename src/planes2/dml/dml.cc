...
namespace dml {
  ...
  void VM::run() noexcept {
    ...
    while(!WindowShouldClose()) {
      ...
      uint32_t async_test = pgtext[CURTASK.pc];
      async_test <<= 8;
      async_test |= pgtext[CURTASK.pc + 1];
      ...
      std::cout << "THREAD " << std::hex << sch.c_task << " @" << CURTASK.pc << ": " << std::hex << opcode << "\n";
